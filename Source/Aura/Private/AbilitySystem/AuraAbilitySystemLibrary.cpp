...
FGameplayEffectContextHandle UAuraAbilitySystemLibrary::ApplyDamageEffect(FDamageEffectParams DamageEffectParams)
{
	...
	const FGameplayEffectSpecHandle SpecHandle = SourceASC->MakeOutgoingSpec(DamageEffectParams.DamageGameplayEffectClass, DamageEffectParams.AbilityLevel, EffectContextHandle);
...
FVector UAuraAbilitySystemLibrary::GetDeathImpulse(const FGameplayEffectContextHandle& EffectContextHandle)
{
	if (const FAuraGameplayEffectContext* AuraEffectContext = static_cast<const FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		return AuraEffectContext->GetDeathImpuse();
	}
...
void UAuraAbilitySystemLibrary::SetIsSuccesfulDebuff(FGameplayEffectContextHandle& EffectContextHandle,
...
void UAuraAbilitySystemLibrary::GetClosesTargets(int32 MaxTargets, const TArray<AActor*>& Actors, TArray<AActor*>& OutClosesTargets, const FVector& Origin)
{
	...
	while (NumTargetsFound < MaxTargets)
	{
		...
		OutClosesTargets.AddUnique(ClosestActor);
		++NumTargetsFound;
	}
}
