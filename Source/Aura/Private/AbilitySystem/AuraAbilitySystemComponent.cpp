...
void UAuraAbilitySystemComponent::AbilityInputTagHeld(const FGameplayTag& InputTag)
{
	...
	for (FGameplayAbilitySpec AbilitySpec : GetActivatableAbilities())
	{
		...
		{
			AbilitySpecInputPressed(AbilitySpec); // Telling the ability: Your ability is being pressed!
...
void UAuraAbilitySystemComponent::AbilityInputTagReleased(const FGameplayTag& InputTag)
{
	...
	for (FGameplayAbilitySpec AbilitySpec : GetActivatableAbilities())
	{
		...
		{
			AbilitySpecInputReleased(AbilitySpec); // Telling the ability: Your ability is being released!
...
void UAuraAbilitySystemComponent::ServerEquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			...
			if (!AbilityHasAnySlot(*AbilitySpec)) // AbilityHasAnySlot
			{
...
void UAuraAbilitySystemComponent::ServerSpendSpellPoint_Implementation(const FGameplayTag& AbilityTag)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		else if (Status.MatchesTagDepth(GameplayTags.Abilities_Status_Equipped) || Status.MatchesTagExact(GameplayTags.Abilities_Status_Unlocked))
		{
			AbilitySpec->Level += 1;
